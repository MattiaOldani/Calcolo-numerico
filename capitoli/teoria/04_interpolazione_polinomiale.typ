...
*Dimostrazione*: assumiamo per assurdo che esistano due polinomi $P_N (x)$ e $Q_N (x)$ in $PP_N$ tali che $ P_N (x_i) = Q_N (x_i) = y_i quad forall i = 0, dots, N . $ Ma allora $P_N (x) - Q_N (x) in PP_N$ e $P_N (x_i) - Q_N (x_i) = 0 quad forall i = 0, dots, N$, cioè quel polinomio si annulla in $N+1$ punti distinti.

Questo implica che $P_N (x) - Q_N (x) = 0 forall x in RR$ perché per il teorema fondamentale dell'algebra, l'unico polinomio di grado $N$ che si annulla in $N+1$ punti distinti è il polinomio banale identicamente nullo, quindi $P_N (x) = Q_N (x)$ e quindi $P_N (x)$ è unico.

Per dimostrare l'esistenza si procede in maniera costruttiva tramite *metodo di Vandermonde* o *metodo di Lagrange*.

_Metodo di Vandermonde_: il generico polinomio è $ P_N (x) = sum_(j = 0)^N c_j x^j = c_0 + c_1 x + dots + c_N x^N . $ Se imponiamo il passaggio per i punti otteniamo un sistema lineare del tipo $ cases(c_0 + c_1 x_0 + dots + c_N x_0^N = y_0, dots, c_0 + c_1 x_N + dots + c_N x_N^N = y_N) . $ Questo metodo è implementato dalla funzione _polyfit_ di Matlab.
...
_Metodo di Lagrange_: definiamo $N+1$ polinomi di Lagrange $L_i (x) quad i = 0, dots, N$ che soddisfano le seguenti proprietà:
- $L_i (x) in PP_N$;
...
*Teorema*: siano $x_0, dots, x_N$ un insieme di $N+1$ nodi distinti, $x eq.not x_i quad forall i = 0, dots, N$ e $f in C^(N+1) (I_x)$, dove $I_x$ più piccolo intervallo chiuso e limitato contenente i nodi $x_0, dots, x_N, x$.
